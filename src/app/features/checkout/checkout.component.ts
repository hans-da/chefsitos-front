      ...
      <div class="max-w-3xl mx-auto">
        ...
        <div class="text-center mb-16 animate-in">
          <div class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-full mb-6">
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em]">Checkout · Paso Final</span>
          </div>
        ...
        @if (!cart()) {
          <div class="bg-white rounded-3xl p-12 text-center border border-gray-100 mb-8">
            ...
            <p class="text-gray-500 mb-4">No hay un carrito activo. Quizás ya completaste tu compra.</p>
            <div class="flex gap-4 justify-center">
              <a routerLink="/mis-ordenes" class="text-indigo-600 font-bold hover:underline">Ver mis órdenes</a>
              <a routerLink="/catalogo" class="text-indigo-600 font-bold hover:underline">Ir al catálogo</a>
            </div>
          </div>
        } @else {
          <!-- Resumen del carrito -->
          ...
          <form [formGroup]="checkoutForm" (ngSubmit)="processOrder()" class="space-y-8">
            <div class="bg-white rounded-[3rem] p-12 border border-gray-100 shadow-2xl shadow-indigo-300/30 relative overflow-hidden">
              <div class="absolute -top-40 -left-40 w-96 h-96 bg-indigo-600 rounded-full blur-[100px] opacity-10 pointer-events-none"></div>
              <h2 class="text-xs font-black uppercase tracking-[0.3em] text-gray-400 mb-10 italic underline decoration-indigo-200 underline-offset-8 relative z-10">01 Información del Destinatario</h2>
              <div class="space-y-6">
            ...
            <div class="bg-white rounded-[3.5rem] p-12 shadow-2xl shadow-indigo-300/50 border border-gray-100 text-gray-900 relative overflow-hidden">
              <div class="absolute -top-24 -right-24 w-80 h-80 bg-indigo-600 rounded-full blur-[80px] opacity-10 pointer-events-none"></div>

              <h2 class="text-xs font-black uppercase tracking-[0.3em] text-indigo-600 mb-8 italic relative z-10">02 Confirmar Orden</h2>
              
              <div class="relative z-10 space-y-4 text-gray-500 text-sm mb-10">
                <p>Al confirmar, se creará tu orden en el sistema con el estado <strong class="text-gray-900">PENDIENTE</strong>. El administrador la procesará y podrás seguir su avance desde <strong class="text-gray-900">Mis Órdenes</strong>.</p>
              </div>
              ...
              @if (processing()) {
                ...
                <button type="submit" [disabled]="checkoutForm.invalid || !cart()" class="w-full mt-4 py-7 bg-indigo-600 text-white rounded-[2rem] font-black uppercase tracking-[0.4em] text-xs hover:bg-indigo-700 hover:scale-[1.02] transition-all shadow-xl active:scale-95 disabled:opacity-40 disabled:scale-100 disabled:bg-gray-400 relative z-10">
                  Confirmar Orden · {{ cart()?.total | currency:cart()?.moneda }}
                </button>
              }
              <p class="text-center text-[8px] font-black text-gray-400 uppercase tracking-[0.4em] mt-8 relative z-10">Procesado por UAMIShop</p>
            </div>
          </form>
        }
      </div>
...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit() {
    // Si no hay carrito, no redirigir inmediatamente - la UI mostrará un mensaje elegante
    // Si el carrito no está en checkout, intentar iniciar checkout
    const current = this.cart();
    if (current && current.estado === 'ACTIVO') {
      // El carrito aún no está en checkout, intentar iniciarlo
      this.cartService.startCheckout().subscribe({
        error: () => {
          this.notification.error('Error al iniciar el proceso de checkout');
  ...
  processOrder() {
    ...
    // Flujo real:
    // 1. Crear orden desde carrito (backend)
    // 2. Completar checkout del carrito (backend)
    // Si paso 1 tiene éxito pero paso 2 falla → la orden ya existe, navegar a mis órdenes
    this.orderService.createOrderFromCart(current.carritoId, direccion).pipe(
      switchMap(() =>
        this.cartService.completeCheckout().pipe(
          catchError(() => {
            // La orden se creó pero completar checkout falló
            // Limpiar carrito local de todas formas
            this.cartService.clearLocalCart();
            this.notification.info('Tu orden fue creada. El carrito se cerrará automáticamente.');
            return of(null);
          })
        )
      )
